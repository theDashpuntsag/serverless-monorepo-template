...
import type { LbdFuncResponse, GenericFuncResponse as Response } from '@custom-repo/global-types';
import { logger } from '../utils';

const lambdaClient = new LambdaClient({ region: 'ap-southeast-1' });

// Default (RequestResponse)
export async function invokeLambdaFn<T>(fnName: string, payload: unknown): Promise<Response<T>>;
...
export async function invokeLambdaFn<T>(fnName: string, payload: unknown, iType: 'Event'): Promise<void>;

// DryRun → validates only
export async function invokeLambdaFn<T>(fnName: string, payload: unknown, iType: 'DryRun'): Promise<Response<T>>;
