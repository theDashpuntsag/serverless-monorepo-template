 ...
 * ```
 */
export function extractQueryParamsFromEvent(event: EventType, query: DynamoQueryRequest): DynamoQueryRequest {
  ...
  if (Object.keys(queryParams).length === 0) {
    const parseResult = dynamoQueryRequestSch.safeParse(query);
  ...
  if (!isPkeyExists && queryParams.index === query.indexName) {
    ...
    const parseResult = dynamoQueryRequestSch.safeParse(params);
    if (!parseResult.success) {
      const errorDetails = parseResult.error.issues.map((err) => `${err.path}: ${err.message}`).join(', ');
